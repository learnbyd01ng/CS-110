...
HTTPProxy::HTTPProxy(int argc, char *argv[]) throw (HTTPProxyException) :
  ...
  try {
    ...
    scheduler.setProxyType(usingProxy, proxyServer, proxyPortNumber);
    createServerSocket();
...
static const string kUsageString = 
   "Usage: proxy [--port <port-number>] [--proxy-server <proxy-server> [--proxy-port <port-number>]]";
void HTTPProxy::configureFromArgumentList(int argc, char *argv[]) throw (HTTPProxyException) {
  ...
  while (true) {
    ...
    case 'r':
      ...
      break;
    default:
      oss << "Unrecognized or improperly supplied flag passed to command line." << endl;
      oss << kUsageString;
  ...
  argc -= optind;   
  if (argc > 0) {
    oss << "Too many arguments passed to proxy executable." << endl;
    oss << kUsageString;
    throw HTTPProxyException(oss.str());
  }
