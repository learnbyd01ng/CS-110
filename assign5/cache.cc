...
static const unsigned short kNumLocks = 1001;
HTTPCache::HTTPCache() {
  const char *homeEnvString = getenv("HOME");
  if (homeEnvString == NULL) 
    throw HTTPCacheConfigException("HOME environment variable must be set by the shell you're using.");
  string homeEnv(homeEnvString);
  cacheDirectory = homeEnv + "/" + kCacheSubdirectoryPrefix;
...
void HTTPCache::lockRequest(HTTPRequest& request) {
  size_t lockNum = hashRequest(request) % kNumLocks;
  locks[lockNum].lock();
}

void HTTPCache::unlockRequest(HTTPRequest& request) {
  size_t lockNum = hashRequest(request) % kNumLocks;
