...
static int IsSameFile(char *pathname1, char *pathname2, char *chksum1, char *chksum2);
...
char *Pathstore_path(Pathstore *store, char *pathname, int discardDuplicateFiles) {
  ...
  store->elementList = e;
  int err = chksumfile_bypathname((struct unixfilesystem *)(store->fshandle), pathname,e->chksum);
  if (err < 0) {
...
static int SameFileIsInStore(Pathstore *store, char *pathname) {
  ...
  // Find the checksum for pathname 1 time rather than for the whole while loop.
  char newpath_chksum[CHKSUMFILE_SIZE];
  int err = chksumfile_bypathname(fs, pathname, newpath_chksum);
  if (err < 0) {
    fprintf(stderr,"Can't checksum path %s\n", pathname);
    return 0;
  }
  ...
  while (e) {
    if (IsSameFile(pathname, e->pathname, newpath_chksum, e->chksum)) {
